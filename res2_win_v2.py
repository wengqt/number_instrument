...
def makedilateMask(src,l_d):
    ...
    if p2==-1:
        if wid < 500 and wid > 200 and l_d<100:
            kel=  cv2.getStructuringElement(cv2.MORPH_RECT, (6, 25))
            print('当前第二个参数值为15')
        elif wid > 500 and wid < 1000 and l_d<100:
            kel=  cv2.getStructuringElement(cv2.MORPH_RECT, (8, 30))
            print('当前第二个参数值为20')
        elif wid > 1000 and wid<1400 and l_d<100:
            kel=  cv2.getStructuringElement(cv2.MORPH_RECT, (9, 32))
            print('当前第二个参数值为20')
        elif wid > 1400 and l_d<100:
            kel =  cv2.getStructuringElement(cv2.MORPH_RECT, (9, 34))
            print('当前第二个参数值为20')
        elif wid < 200:
            kel= kernel5
            print('当前第二个参数值为5')
        ...
        if l_d>100:
            if wid > 1400:
                ...
                kel = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 32))
                print('当前第二个参数值为16')
            elif  wid > 620 and wid < 1000:
                kel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 30))
                print('当前第二个参数值为14')
            elif wid < 620 and wid > 200:
                kel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 30))
                print('当前第二个参数值为12')
    elif p2!=-1:
        kel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, p2))
        print('当前第二个参数值为'+str(p2))
...
def ssd2_resize(img,img_height,img_width):
...
def parse_box(res_arr):
    if len(res_arr)==0:
        print('没有识别到数字')
        sys.exit(0)

    res_arr = sorted(res_arr, key=lambda x: ((x[5] + x[3])/2))
    new_arr =[]
        ...
        if len(new_arr)==0 :
            ...
            for line in new_arr:
                ...
                for it in line:
                    y_ms += (it[5] + it[3])/2
                avg_y = y_ms/len(line)
                if (item[5] + item[3])/2 <avg_y + 20 and (item[5] + item[3])/2 > avg_y -20:
                    line.append(item)
                    i_have = True
                    break
            if i_have == False:
                new_arr.append([item])
    the_arr=[]
    for a_line in new_arr:
        # average_rate = [cv2.countNonZero(test_single_channel[x[3]:x[5],x[2]:x[4]])/((x[5]-x[3])*(x[4]-x[2])) for x in a_line]
        average_area = [((x[5]-x[3])*(x[4]-x[2])) for x in a_line]
        # print(average_area)
        the_line=[]
        stand = sum(average_area)/len(average_area)*0.6
        for ind in range(len(average_area)):
            # print(a_line[ind][0])
            if average_area[ind]<stand and a_line[ind][0]!='-':
                continue
            else:
                the_line.append(a_line[ind])
        the_arr.append(the_line)

    sort_arr=[]
    for a_line in the_arr:
        _line = sorted(a_line, key=lambda x : x[2])
        new_line =[]
        skip = False
        for _i in range(len(_line)-1):
            # print(_line,_i,skip)
            if skip ==False:
                # print(_line[_i][4],_line[_i+1][2])
                if _line[_i][4] > _line[_i+1][2]:
                    if _line[_i+1][4] <= _line[_i][4]:
                        # print(2222)
                        new_line.append(_line[_i+1]) if _line[_i+1][1]>_line[_i][1] else new_line.append(_line[_i])
                        skip = True
                    elif _line[_i+1][4] > _line[_i][4]:
                        bbb = _line[_i][4] - _line[_i+1][2]
                        # print(111111111,_i,bbb,0.7*(_line[_i][4]-_line[_i][2]),0.7*(_line[_i+1][4]-_line[_i+1][2]))
                        if bbb>0.7*(_line[_i][4]-_line[_i][2]) or bbb>0.7*(_line[_i+1][4]-_line[_i+1][2]):
                            new_line.append(_line[_i + 1]) if _line[_i + 1][1] > _line[_i][1] else new_line.append(_line[_i])
                            skip = True
                        else:
                            new_line.append(_line[_i])
                            skip = False
                ...
                continue
        if skip == False:
            new_line.append(_line[len(_line)-1])


        sort_arr.append(new_line)
    print('排序结果：', sort_arr)
...
def compare_loc(num_arr,loc):
    print(loc)
    print(num_arr)
    _index=0
    for item in num_arr:
        if (item[2]+item[4])/2-5>loc[0]:
            break
        else:
            _index+=1

    num_string = [str(x[0]) for x in num_arr]
    num_string.insert(_index,'.')
    return ''.join(num_string)

def cut_line(num_arr,_img):
    ind=0
    line_image=[]
    res_arr=[]
    for _line in num_arr:
        ind += 1
        y_maxs = [x[5] for x in _line]
        y_mins = [x[3] for x in _line]
        max_y = max(y_maxs)
        min_y = min(y_mins)
        # print(max_y,min_y)
        one_image = _img[min_y:max_y,:]
        line_image.append(one_image)


        dot_img_arr=[]
        dot_img_arr.append(cv2.imread('./dot1.jpg', 0))
        dot_img_arr.append(cv2.imread('./dot2.jpg', 0))
        dot_img_arr.append(cv2.imread('./dot3.jpg', 0))
        # _, dot_img = cv2.threshold(dot_img, 27, 255,cv2.THRESH_BINARY)
        # dot_img = cv2.GaussianBlur(cv2.resize(dot_img,(10,10)),(3,3),1)
        #
        # imwrite('./dot1.jpg',dot_img)
        if os.path.exists('./dot1.jpg') == False or os.path.exists('./dot2.jpg') == False or os.path.exists('./dot3.jpg') == False:
            print('err', '缺失小数点模板')
            sys.exit(0)

        dots = []
        for dot_img in dot_img_arr:
            for i in range(7):
                dots.append(cv2.resize(dot_img, (int(10 / (1 + 0.1 * i)), int(10 / (1 + 0.1 * i)))))
            for i in range(7):
                dots.append(cv2.resize(dot_img, (int(10 * (1 + 0.1 * i)), int(10 * (1 + 0.1 * i)))))
        ...
        for i in range(len(dots)):
            (d_h,d_w)=dots[i].shape
            # _, a_num =  cv2.threshold(a_num, 127, 255,cv2.THRESH_BINARY)
            one_channel = one_image
            # one_channel = cv2.GaussianBlur(one_image,(3,3),0)
            res = cv2.matchTemplate(one_channel, dots[i], cv2.TM_CCOEFF_NORMED)
            ...
            # print(loc)
            for pt in zip(*loc[::-1]):
                # print(pt[1],pt[0],res[pt[1]][pt[0]],one_image.shape[0]/2)
                if res[pt[1]][pt[0]] > tmp_dot_max and pt[1]>one_image.shape[0]/2-10 :
                    tmp_dot_max = res[pt[1]][pt[0]]
                    might_loc =(pt[0],pt[1])

        cv2.rectangle(one_image, might_loc, (might_loc[0] + 10, might_loc[1] + 10), 255, 1)
...
def process1(path,outPath = './result.txt'):
    ...
    for b in blocks_sets:
        ...
        print('数字检测')
        num_arr,img,single_img = num_detection(correct)
        # print(num_arr)
        print('识别小数点')
        res_list = cut_line(num_arr,single_img)
        result_arr +=res_list
    f = open(outPath, 'a')
    f.write('\n' + path)
