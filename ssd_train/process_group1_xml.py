...
def predict_num_area(src):
    ...
    for box in max_arr[:1]:
        ...
        write_xml(img_name, [int((xmin)*scale/4.21),int((ymin-t_)*scale/4.21),int((xmax)*scale/4.21),int((ymax-t_)*scale/4.21)])
...
def write_xml(imgname,arr):
    ...
    etree.ElementTree(anno_tree).write('./group1_expand/xml/' + imgname + ".xml", pretty_print=True)
...
def process1(path,outPath = './result.txt'):
    ...
    # print('1.获得数字区域')
    area_mask = predict_num_area(train_o)
...
if __name__ == '__main__':
    ...
    for img_index in range(0,1014):
        img_path = './group1_expand/image' + str(img_index) + '.jpg'
        img_name = 'img' + str(img_index) + '.jpg'
