...
def cutImage(findContoursImg, dst, origin):
    ...
    # 绘制结果
    cv2.imwrite('./v5/red_contours.jpg', cont_img)
    ...
    cv2.imwrite('./v5/' + dst, cut)
    return cut

def cutImage1(findContoursImg, dst, origin):
    _, contours, hierarchy = cv2.findContours(findContoursImg, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    # 绘制轮廓
    c = sorted(contours, key=cv2.contourArea, reverse=True)
    cont_img = origin.copy()
...
def correctAngle(src):
    canny = cv2.Canny(cv2.GaussianBlur(src, (5, 5), 0), 100, 200)
    cv2.imwrite('./v5/numZonecanny.jpg', canny)
    canny = cv2.dilate(canny, kernel3, iterations=1)
    cv2.imwrite('./v5/numZonecannyDilate.jpg', canny)
    ...
    cv2.imwrite('./v5/numZonecorrect.jpg', correct)
    return correct



def splitNum(num_Img, index):
    cv2.imwrite('./v5/split/split' + str(int(index)) + 'Img.jpg', num_Img)
    num1closed = cv2.morphologyEx(num_Img, cv2.MORPH_CLOSE, kernel5)
    cv2.imwrite('./v5/split/split' + str(int(index)) + 'closed.jpg', num1closed)
    ...
    # cv2.imwrite('./v5/num1Vertical.jpg', num1Vertical)
    ...
    for i in range(0, len(num1points), 2):
        ...
        cv2.imwrite('./v5/split/split' + str(index) + '_' + str(int(i / 2)) + '.jpg',
...
def separateNum(correct_src,iii=11):
    ...
    for j in range(len(nums)):
        ...
        for oneimg in nums[j]:
            one_index += 1
            dot_line = cv2.HoughLines(oneimg, 1, np.pi / 180, 40)
            l_num = 0
            ang = 0
            for ls in dot_line:
                for line in ls:
            ...
            cv2.imwrite('./v5/split/correct' + str(j) + '_' + str(one_index) + '.jpg', oneimg_cp)
    ...
    for j in range(len(nums)):
        ...
        for oneimg in nums[j]:
            ...
            cv2.imwrite('./v5/res/' + str(iii) + '_' + str(j) + '_' + str(one_index) + '.jpg', oneimg)
            arr.append('./v5/res/' + str(iii) + '_' + str(j) + '_' + str(one_index) + '.jpg')
        imgList.append(arr)
    print('dots:',dots)
    return imgList, dots
...
def processImg(dir):
    ...
    cv2.imwrite('./v5/points.jpg', im_zeros)

    instrumentArea = cutImage(im_zeros, 'cut1.jpg', train)

    cutImg,instrumentArea = v4_kmeans.gray_cluster(instrumentArea)
    ...
    # cv2.imwrite('./v5/sobelx.jpg', cutImg)
    ...
    # cv2.imwrite('./v5/cutadapt.jpg', cutImg)
    # cutImg = cv2.morphologyEx(cutImg, cv2.MORPH_OPEN, kernel4)
    # cv2.imwrite('./v5/cutopened.jpg', cutImg)
    # cutImg = cv2.erode(cutImg, kernel3, iterations=1)
    # cv2.imwrite('./v5/cuterode.jpg', cutImg)
    # cutImg = cv2.dilate(cutImg, kernel2, iterations=3)
    # cv2.imwrite('./v5/cutdilate.jpg', cutImg)
    cutImg1 = cv2.dilate(cutImg, kernel1, iterations=2)
    cv2.imwrite('./v5/444dilate.jpg', cutImg1)
    cutImg = cutImage1(cutImg1, 'numZone.jpg', cutImg)
    # cutImg,unused = v4_kmeans.gray_cluster(cutImg,2)
    cutImg = cv2.erode(cutImg, kernel3, iterations=1)
    cv2.imwrite('./v5/222erode.jpg', cutImg)
    # 倾斜矫正
